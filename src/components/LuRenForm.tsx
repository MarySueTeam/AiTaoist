...
import { HelpCircle, Calendar, Clock, Hexagon } from 'lucide-react';
import type { ToneMode } from '../types/toneMode';

interface LuRenFormProps {
  onSubmit: (data: { question: string; date: string; time: string; toneMode: ToneMode; isHarshMode: boolean }) => void;
  isLoading: boolean;
}

export default function LuRenForm({ onSubmit, isLoading }: LuRenFormProps) {
  ...
  return (
    ...
    >
      <div className="text-center mb-10">
        <h2 className={`text-3xl font-light tracking-[0.2em] mb-2 transition-colors duration-500 ${isHarshMode ? 'text-red-100' : isSweetMode ? 'text-rose-800' : 'text-ink'}`}>大六壬神课</h2>
        <p className={`text-sm tracking-widest transition-colors duration-500 ${isHarshMode ? 'text-red-200/60' : isSweetMode ? 'text-rose-600/70' : 'text-ink/40'}`}>袖里乾坤，一语道破</p>
      </div>
